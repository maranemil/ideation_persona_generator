...
function generatePersona() {
   ...
   $strout    = "";
   $genderout = $anrede[rand(0, count($anrede) - 1)];
   $strout    .= $genderout;
   if ($genderout == "Herr") {
	  $strout .= $vorname_m[rand(0, count($vorname_m) - 1)];
   }
   else {
	  $strout .= $vorname_f[rand(0, count($vorname_f) - 1)];
   }
   $strout .= $nachname[rand(0, count($nachname) - 1)];
   $strout .= ", ist momentan in ";
   $strout .= $location[rand(0, count($location) - 1)];
   ...
   $strout .= " weil seine ( ";
   $strout .= $subst_m[rand(0, count($subst_m) - 1)] . " / ";
   $strout .= $subst_f[rand(0, count($subst_f) - 1)] . " / ";
   $strout .= $subst_n[rand(0, count($subst_n) - 1)];
   $strout .= " ) ... ";
   return $strout;
}
